...
//!
//! This module provides a persistent storage implementation using SQLite via rusqlite.
...
impl AddressCacheDatabase for SqliteDatabase {
    ...
    fn load(&self) -> Result<Vec<CachedAddress>> {
        ...
        let rows = stmt.query_map([], |row| {
            ...
            let script_hash =
                Hash::from_byte_array(script_hash.try_into().map_err(|_| -> SqliteError {
            ...
            let transactions: rusqlite::Result<Vec<Txid>, SqliteError> = transactions
                .chunks_exact(32)
                .map(|chunk| {
                    Ok(Txid::from_byte_array(chunk.try_into().map_err(
                        |_| -> SqliteError {
                            SqliteError::FromSqlConversionFailure(
                                0,
                                Type::Blob,
            ...
            let utxos: rusqlite::Result<Vec<OutPoint>, SqliteError> = utxos
                .chunks_exact(36)
                .map(|chunk| {
                    let txid = Txid::from_byte_array(chunk[..32].try_into().map_err(
                        |_| -> SqliteError {
                            SqliteError::FromSqlConversionFailure(
                                0,
                                Type::Blob,
                    ...
                    let vout = u32::from_le_bytes(chunk[32..36].try_into().map_err(
                        |_| -> SqliteError {
                            SqliteError::FromSqlConversionFailure(
                                0,
                                Type::Blob,
        ...
        let ret = rows.collect::<rusqlite::Result<Vec<CachedAddress>>>()?;
    ...
    fn list_transactions(&self) -> Result<Vec<Txid>> {
        ...
        let rows = stmt.query_map([], |row| {
            ...
            Ok(Txid::from_byte_array(bytes.try_into().map_err(
                |_| -> SqliteError {
                    SqliteError::FromSqlConversionFailure(
                        0,
                        Type::Blob,
        ...
        let ret: rusqlite::Result<Vec<Txid>> = rows.collect();
        Ok(ret?)
    }
}
