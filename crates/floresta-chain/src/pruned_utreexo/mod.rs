...
pub trait UpdatableChainstate {
    ...
    /// Reconsiders a previously invalidated block (via `invalidate_block`). If the
    /// reconsidered chain has more work than the current best chain, triggers a reorg.
    /// Blocks that failed consensus validation (`ValidationFailed`) cannot be reconsidered.
