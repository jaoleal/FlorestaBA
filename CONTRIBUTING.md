...
This project follows [Semantic Versioning (semver)](https://semver.org/). However, we maintain **separate version tracks** for different components:

- **Application binaries** (`florestad` and `floresta-cli`): These follow the main project release cycle and share a version number (e.g., `0.8.x`).
- **Library crates** (the crates re exported in `floresta`): These are versioned independently based on their public API stability (e.g., `0.4.x`).

This separation exists because library crates published to crates.io must be versioned according to their API changes, which don't always align with application feature releases.

### Signed Binaries.

A release should contain pre-built binaries available on github's asset page. They **must** be GPG signed, and have a list of hashes for each asset.

Building and signing is done with the help of a just recipe created only for it, this allows for the project to have a certain reproducibility for the release assets.

Creating a release can be achieved with `just release`, it will run `just build-release` and `just release-sign` right after, which by itself will run `sha256sum` on the binaries and sign them using `gpg`.

This will generate two files unde `contrib/dist/signed_binaries`, one text file containing the hashes and their accordingly binaries

```text
florestad.{version} {hash}

floresta-cli.{version} {hash}
```

and another `.asc` one, with same name, containing your signature.

There may have others `.asc` named with emails, these are the ones signed by `Floresta Team`, you should be able to check them somehow.

TODO: how to fucking check this in a decent manner.
